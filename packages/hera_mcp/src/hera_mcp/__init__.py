...
Her own tools — ``ask``, ``remember``, ``forget``, ``note``, ``skill``, ``search``, the
...
and everything in it is a statement about what Hera can do: what ``remember`` is for, the three
occasions ``ask`` is worth stopping a turn for, the sentence the model reads before deciding to
call ``note``. Those change for reasons that have nothing to do with subprocess lifetimes, and
in v0.3 this is what gets served over a transport of its own so Claude Code can attach to her.
